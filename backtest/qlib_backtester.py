...
def _compute_cross_sectional_ic_metrics(
    ...
    """Compute daily cross-sectional IC / RankIC on forward close returns."""
    ...
    dates = common_dates
    current_mask = np.ones(len(dates), dtype=bool)
    if start_date:
        current_mask &= dates >= pd.Timestamp(start_date)
    if end_date:
        current_mask &= dates <= pd.Timestamp(end_date)

    valid_future_mask = np.zeros(len(dates), dtype=bool)
    if label_forward_days < len(dates):
        valid_future_mask[: len(dates) - label_forward_days] = True
        if end_date:
            future_dates = dates[label_forward_days:]
            valid_future_mask[: len(dates) - label_forward_days] &= future_dates <= pd.Timestamp(end_date)

    eval_mask = current_mask & valid_future_mask
    ...
    for idx in np.where(eval_mask)[0]:
        ...
        if x.nunique(dropna=True) < 2 or y.nunique(dropna=True) < 2:
            # Constant cross-sections make Pearson/Spearman undefined and only add warning noise.
            continue
    ...
    rank_ic_mean = float(np.mean(rank_ic_values)) if rank_ic_values else 0.0
    ...
    return {
        ...
        "rank_icir": round(rank_ic_mean / (rank_ic_std + 1e-12), 6) if len(rank_ic_values) > 1 else 0.0,
        "n_ic_days": len(ic_values),
    }
...
def compute_portfolio_ir(
    ...
    ic_metrics = _compute_cross_sectional_ic_metrics(
    ...
    return {
        ...
        'n_ic_days': ic_metrics['n_ic_days'],
