"""Legacy local GRPO training script for AlphaAgentEvo.

The Kaggle path now defaults to `deploy/v2/*.sh`.
This file is kept for local experiments with trl.GRPOTrainer.
...
def _get_factor_tool(backtest_url: str = "http://localhost:8002") -> FactorTool:
    global _factor_tool
...
def resolve_torch_dtype(dtype_name: str | None) -> torch.dtype | str | None:
    """Map a config-friendly dtype string to a torch dtype."""
...
def main():
    ...
    elif model_config.get("attn_implementation"):
    ...
    backtest_url = config.get("backtest", {}).get("url", "http://localhost:8002")
