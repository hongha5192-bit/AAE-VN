...
BACKTEST_URL = "http://localhost:8002"
...
class FactorTool:
    ...
    def _resolve_endpoints(self) -> tuple[str, str]:
        """Return (evaluate_factor_url, backtest_url)."""
        url = self.backtest_url
        if url.endswith("/evaluate_factor"):
            base = url[: -len("/evaluate_factor")]
            return url, f"{base}/backtest"
        if url.endswith("/backtest"):
            base = url[: -len("/backtest")]
            return f"{base}/evaluate_factor", url
        return f"{url}/evaluate_factor", f"{url}/backtest"
    ...
    @staticmethod
    def _short_text(resp: requests.Response, max_chars: int = 220) -> str:
    ...
    def evaluate(self, factor_name: str, factor_expr: str,
        ...
        """
        eval_url, backtest_url = self._resolve_endpoints()
        payload_eval = {"factor_name": factor_name, "factor_expr": factor_expr}
        if period:
            payload_eval["period"] = period

        # 1) Preferred API: /evaluate_factor
        try:
            resp = requests.post(
                eval_url,
                json=payload_eval,
            ...
            if resp.status_code == 200 and data and "success" in data:
                return {
                    "success": bool(data.get("success", False)),
                    "ir": float(data.get("ir", 0.0) or 0.0),
                    "ic_mean": float(data.get("ic_mean", 0.0) or 0.0),
                    "error": data.get("error"),
                }
            eval_err = f"/evaluate_factor status={resp.status_code} body={self._short_text(resp)}"
        except Exception as e:
            eval_err = f"/evaluate_factor exception: {str(e)[:200]}"

        # 2) Fallback API: /backtest (used by deploy/api_server_verl.py)
        payload_backtest = {
        ...
        try:
            resp = requests.post(
                backtest_url,
                json=payload_backtest,
                timeout=120,
            )
            data = self._safe_json(resp)
            if resp.status_code == 200 and data and data.get("data"):
                metrics = data["data"].get("metrics", {})
                return {
                    "success": True,
                    "ir": float(metrics.get("Information_Ratio_with_cost", 0.0) or 0.0),
                    "ic_mean": float(metrics.get("IC", 0.0) or 0.0),
                    "error": None,
                }
            ...
            return {
                ...
                "ir": 0.0,
                "ic_mean": 0.0,
                "error": f"{eval_err}; /backtest status={resp.status_code} detail={detail if detail is not None else self._short_text(resp)}",
            }
        except Exception as e:
            return {
                "success": False,
                "ir": 0.0,
                "ic_mean": 0.0,
                "error": f"{eval_err}; /backtest exception: {str(e)[:200]}",
            }
