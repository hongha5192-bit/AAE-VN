...
def KURT(df:pd.DataFrame):
    """计算横截面峰度"""
    return df.groupby('datetime').transform('kurt')

@datatype_adapter
def MAX(df:pd.DataFrame):
    """计算横截面最大值"""
    return df.groupby('datetime').transform('max')

@datatype_adapter
def MIN(df:pd.DataFrame):
    """计算横截面最小值"""
    return df.groupby('datetime').transform('min')
...
def TS_CORR(df1:pd.Series, df2: np.ndarray | pd.Series, p:int=5):
    ...
    if isinstance(df2, np.ndarray) and p != len(df2):
        ...
        for instrument, group in df1.groupby('instrument'):
            df2_group = df2.xs(instrument, level='instrument')
            corr_vals = group.rolling(p, min_periods=2).corr(df2_group)
            result.loc[group.index] = corr_vals.values
        return result
...
def TS_COVARIANCE(df1:pd.DataFrame, df2:pd.DataFrame, p:int=5):
    ...
    if isinstance(df2, np.ndarray) and p != len(df2):
        ...
        for instrument, group in df1.groupby('instrument'):
            df2_group = df2.xs(instrument, level='instrument')
            cov_vals = group.rolling(p, min_periods=2).cov(df2_group)
            result.loc[group.index] = cov_vals.values
        return result
...
def INDUSTRY_NEUTRALIZE(signal_df: pd.DataFrame, industry_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Expressions can produce inf/-inf before neutralization; treat those as missing
    # so group means and subtract stay finite and do not flood logs with warnings.
    signal_clean = signal_vals.replace([np.inf, -np.inf], np.nan)

    dt = signal_df.index.get_level_values('datetime')

    # Build a temporary frame with reset index to avoid ambiguity
    tmp = pd.DataFrame({
        'signal': signal_clean.values,
        'industry': industry_col,
        'dt': dt,
    })

    group_mean = tmp.groupby(['dt', 'industry'])['signal'].transform('mean')
    result_vals = signal_clean.values - group_mean.values
